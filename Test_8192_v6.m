...
%% OR you can choose tx and rx antenna. lets go with first antennas
...
%% Apply precoding
txfd_precoded = zeros(nFFT, 8, nSymbols);
...
for i = 1:nFFT
    T = squeeze(txfd(i,:,:)); 
    txfd_precoded(i, :, :) =  squeeze(W_k(i,:,:)) * T';
end
...
plot(real(t_hat(:, stream_idx, rx_idx)), ...
    imag(t_hat(:, stream_idx, rx_idx)), '.', 'MarkerSize', 20)
grid on;
title('Beamfomed, No interference canceling matrix');
ylim([-2 2]);
xlim([-2 2]);
