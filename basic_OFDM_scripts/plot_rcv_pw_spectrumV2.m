...
for i = 1:length(rx_rotations)
    ...
    for j = 1:length(configs)
        ...
        grid on; grid minor;
        ylim([-20 0]);
        title('Directional response pattern', 'FontSize', FntSize);
        ...
        hold on;
        for rxIndex = 1:length(tx_antennas)
            plot(freq_GHz(startIndex:stopIndex), mag2db(fftshift(pSpectrumArray(rxIndex, startIndex:stopIndex))), 'DisplayName', ['RX Ant ' num2str(tx_antennas(rxIndex))]);
        end
        hold off;
        ...
        title('Receive Power Spectrum (58 GHz ± 0.39 GHz)', 'FontSize', FntSize);
